...
<hr>
조건문 사용 <br>
<c:set var ="aa" value="tar"/>
<c:if test="${aa == 'star'}">
if test1 : aa 같은 <c:out value="${aa}"/>
</c:if>
<br>
<c:if test="${aa != null}">
if test2 : aa 같은 <c:out value="${aa}"/>
</c:if>
<br>
choose 조건문 사용 <br>
<c:choose>
...
<hr>
^^ 문자열 분할  ^^<br>
<c:forTokens var="animal" items="horse,tiger,lion*dog*cat" delims=",*" varStatus="num"> <%--delims는 구분자--%>
...
날짜 : <formatDate value="${now }" type="date"/><br>
날짜 : <formatDate value="${now }" type="both"/><br>
